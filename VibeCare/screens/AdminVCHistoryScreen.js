...
const VCHistoryScreen = () => {
  ...
  const { userId, userName } = route.params;   // 👈 get from navigation
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/get-user-chats/${userId}`); // 👈 new API
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.header}>
        Counseling History for {userName}
      </Text>
