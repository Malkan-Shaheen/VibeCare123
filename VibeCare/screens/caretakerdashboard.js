...
import { View, Text, Image, StyleSheet, TouchableOpacity, ScrollView, BackHandler, Alert } from 'react-native';
import axios from 'axios';
import { API_BASE_URL } from '../config/api';
import { useFocusEffect } from '@react-navigation/native';

const caretakerdashboard = ({ route, navigation }) => {
    const { caretakerId, userId } = route.params || {};
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        console.error('Error fetching caretaker user:', error?.message || error);
        Alert.alert(
          'Connection Error',
  ...
  // ✅ Handle Android hardware back & navigation back
  useFocusEffect(
    React.useCallback(() => {
      const onBackPress = () => {
        navigation.reset({
  ...
  return (
    ...
    onPress={() => {
      navigation.navigate('CaretakerMentalHealthSummaryScreen', { 
        userId,             // ✅ use actual linked userId
        userName: userName,
      });
    }}
  >
    <Text style={styles.buttonText}>📊 View Mental Health Summary</Text>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('AdminVCHistoryScreen', { userId, userName })}
      >
